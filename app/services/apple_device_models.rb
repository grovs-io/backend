# Maps Apple hardware identifiers ("iPhone17,3") to marketing names ("iPhone 16").
#
# Older iOS SDKs map on-device and send marketing names; their table went stale at
# iPhone 15 Pro Max, so newer hardware arrives as raw identifiers. Newer SDKs send
# raw identifiers for everything. Mapping here means a new device is one entry in
# this table instead of an SDK release every integrator has to adopt.
#
# Only strings shaped like identifiers are touched — marketing names from older
# SDKs, Android models, and simulator values ("arm64") pass through untouched.
class AppleDeviceModels
  IDENTIFIER_PATTERN = /\A(?:iPhone|iPad|iPod)\d+,\d+\z/
  ...
  NAMES = {
    ...
    "iPad8,1" => "iPad Pro (11-inch) (1st generation)", "iPad8,2" => "iPad Pro (11-inch) (1st generation)",
    "iPad8,3" => "iPad Pro (11-inch) (1st generation)", "iPad8,4" => "iPad Pro (11-inch) (1st generation)",
    ...
    "iPad6,7" => "iPad Pro (12.9-inch) (1st generation)", "iPad6,8" => "iPad Pro (12.9-inch) (1st generation)",
  ...
  def self.humanize(model)
    return model unless model.is_a?(String) && model.match?(IDENTIFIER_PATTERN)

    NAMES.fetch(model, model)
  end
end
